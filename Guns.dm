...
mob/Admin3/verb/Gun_Power()
	...
	The higher the number the more damage all guns will do.","Options",Gun_Power) as num
	Log(src,"[key] set the gun power mod to [Gun_Power]x")
...
obj/Turret
	...
	proc/Turret_Fire(mob/P)
		...
		if(prison_exit&&z==prison_exit.z&&getdist(src,prison_exit)<=30)
...
obj/items/Gun
	...
	proc/Gun_Fire(mob/P)
		...
		spawn(To_tick_lag_multiple(Delay)) if(P)
			if(ismob(P)) P.attacking=0
			if(src) Firing=0
		if(Bullet) player_view(10,P)<<sound('reflect.ogg',volume=20)
		...
		while(N)
			...
			A.loc=P.loc
			step(A,A.dir)
			spawn(To_tick_lag_multiple(Velocity)) if(A&&A.z)
				var/Dir=A.dir
				if(Spread)
					switch(N)
				...
				step(A,A.dir)
				spawn(To_tick_lag_multiple(Velocity))
					A.dir=Dir
					while(A&&A.z&&!A.deflected)
						step(A,A.dir)
						sleep(To_tick_lag_multiple(Velocity))
obj/items/Ammo
	Cost=3000
	...
	proc/Reload(mob/M,obj/items/Gun/G)
		...
		if(G)
			...
			var/Reload_Delay
			if(ismob(M)) Reload_Delay=To_tick_lag_multiple((100*sqrt(usr.Speed_delay_mult(severity=0.5)))/G.Reload_Speed)
			else Reload_Delay=To_tick_lag_multiple(300/G.Reload_Speed/sqrt(M.Spd))
			Reloading=1
