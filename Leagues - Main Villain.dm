...
mob/proc/Villain_league_damage_multiplier(mob/target)
	if(server_mode!="PVP"||!client||!sagas||villain!=key||!league_list.len||!target||!target.client||!target.league_list.len||\
...
proc/Villain_league_member_count_loop() spawn
	while(1)
...
proc/League_paychecks() spawn
	sleep(600)
	while(1)
		if(sagas && villain && server_mode=="PVP" && villain_league_id)
			var/list/CIDs=new
