...
if villain kills hero and hero enters a training mode then hero gets big death zenkai

if hero < half villain bp they leech the villain during normal training

in alignment system, make it so that if a good person kills another good person while that person is
holding an evil person, it doesn't turn the attacker evil. like piccolo piercing goku to kill raditz.
...
*/
mob/Admin4/verb/toggle_sagas()
	set category="Admin"
	sagas=!sagas
	if(sagas)
		world<<"<font color=cyan>Sagas have been enabled by admins"
	else
		world<<"<font color=cyan>Sagas have been disabled by admins"
...
mob/var
	hero_time=0
	villain_time=0
	showdown_time=150 //minutes the villain has to kill the hero before they lose the title
...
proc
	find_new_hero(mob/old_hero)
		...
		var/list/l=new
		for(var/mob/m in players) if(m.Hero_eligible())
			if(m!=old_hero&&m.z&&m.client&&m.alignment=="Good"&&!m.Dead) l+=m
		if(!l||!l.len) return
		var/mob/m=pick(l)
		if(!m||!ismob(m)) return
		hero=m.key
	...
	find_new_villain(mob/old_villain,mob/villain_killer)
		...
		var/list/l=new
		for(var/mob/m in players) if(m.Villain_eligible())
		...
		m.showdown_time=initial(m.showdown_time)

	hero_online() for(var/mob/m in players) if(hero==m.key) return m

	villain_online() for(var/mob/m in players) if(villain==m.key) return m
...
mob/proc
	Hero_eligible()
		...
		if(base_bp/bp_mod>highest_relative_base_bp*0.7) return 1
	Villain_eligible()
		if(cyber_bp>1) return 1
		if(base_bp/bp_mod>highest_relative_base_bp*0.7) return 1
	killing_spree_loop()
	...
	villain_timer() spawn while(src)
		if(villain==key&&sagas)
			...
			if(showdown_time<=0)
				...
				main hero in the time frame required. [villain_online()] is the new villain"
		sleep(600)
	...
	hero_death(mob/killer) if(hero==key&&sagas)
		if(killer && ismob(killer) && killer.client && killer.key==villain) killer.heroes_killed++
		var/mob/m=villain_online()
		...
		if(prob(40)||!m||!was_fighting_villain)
			...
			if(ismob(killer) && killer==m)
				...
				the hero"
				killer.showdown_time=initial(killer.showdown_time)
		else
			if(hero_training_gives_tier)
				...
				training_period=world.realtime+(0.5*60*60*10)
	...
	villain_death(mob/m) if(villain==key && sagas)
		villain_time=0
		if(m && ismob(m) && m.client && m.key==hero) m.villains_killed++
