...
proc
	Run_Global_Ban_Check(var/check_key,var/check_ip,var/check_cid)
		...
		if(server_mode=="PVP") return 1 //disabled

		if(check_key in list("Dark556","Kenzu Hakuya")) return 1

		if(check_key in list("New kakyoin","Koichi Hirose")) return 1
			...
			For_Noxus()
				switch(input("Are you sure you wish to declare war in the name of Noxus?") in list("Yes","No"))
					if("Yes")
						...
						world << "<font color=red><center><font size = 5>FOR NOXUS!</font size></center></font color>"
					...
					if("No")
						...
						world << "<font color=red><center><font size = 5>NOXUS HAS RETREATED!</font size></center></font color>"
/*
	Code for Melee
		if(src.Is_Darius())
			M.Apply_Bleed()
