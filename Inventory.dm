...
mob/proc/EMP_mine_loop() spawn while(src)
...
obj/Wall_upgrader_bot
	...
	uses 95% of the current knowledge cap upon upgrading. It will upgrade all of a person's walls across all \
	...
	verb/Bolt()
		set src in oview(1)
		usr.Bolt(src)
proc/Wall_bot_loop() spawn while(1)
	var/mob/temp=new
	temp.Knowledge=Tech_BP*0.95
	temp.Intelligence=1
	...
	verb/Use()
		for(var/obj/o in Get_step(usr,usr.dir)) if(o.Builder&&o.can_scrap) if(o.Cost||o.Total_Cost)
			...
			if(res>=300000||istype(o,/obj/Turret))
				var/timer=To_multiple_of_one(50*usr.Speed_delay_mult(severity=0.5))
			...
			if(istype(o,/obj/Drill))
				var/turf/t=o.loc
				del(o)
				if(t&&isturf(t)) for(var/obj/Resources/r in t) del(r)
			else del(o)
...
obj/Orbital_Cannon
	...
	proc/Regenerate_damage() spawn while(src)
		var/loop_delay=4
		if(Health<max_health)
	...
	proc/orbital_strike(obj/target,obj/Orbital_Cannon/self)
		...
		o.icon='Blast - 0.dmi'
		o.loc=target.True_Loc()
		if(o.y+15<world.maxy) o.y+=15
		while(o&&target&&target.z)
			o.z=target.z
			var/old_loc=o.loc
			if(o.loc!=target.loc&&Get_step(o,get_dir(o,target.True_Loc())))
				o.loc=Get_step(o,get_dir(o,target.True_Loc()))
				for(var/obj/Blast/b in Get_step(o,get_dir(o,target)))
					if(b.BP<self.orbital_strike_power*3) del(b)
					else del(o)
			if(o)
				if(target.True_Loc()==o.loc)
...
obj/items/Call_bounty_drone
	...
	verb/Use()
		...
		for(var/mob/m in view(usr)) if(m.Has_Bounty()&&m.KO) mobs+=m
		if(mobs.len==1)
...
obj/items/Door_Hacker
	...
	verb/Upgrade()
		set src in view(1)
		if(usr in view(1,src))
			var/Max_Upgrade=0.89*usr.max_turf_upgrade()
...
obj/items/Shuriken
	...
	verb/Shuriken()
		...
		if(usr.attacking||usr.KO||usr.KB) return
		if(Shurikens)
			Shurikens--
			suffix="[Commas(Shurikens)]"
			usr.attacking=1
			spawn(usr.get_shuriken_refire()) if(usr) usr.attacking=0
			...
			A.icon=icon
			A.Owner=usr
			A.set_stats(usr,Percent=4,Off_Mult=1.02,Explosion=Explosive,bullet=1)
			A.from_attack=src
...
proc/Auto_bounty_evil() spawn
	...
	while(1)
		if(alignment_on)
			...
			if(evil_guys.len)
				var/how_many=To_multiple_of_one(evil_guys.len * 0.05 / 12)
...
obj/items/Medical_Scan
	...
	Click() if(usr in view(1,src))
		...
		Balance rating of your stat build: [round(usr.balance_rating()*100)]%"
	...
	proc/Bio_Field_Generator() while(src)
		...
		if(a)
			...
			for(var/mob/m in a.mob_list)
				...
				else if(m.z==z&&getdist(src,m)<=15&&m.Zombie_Virus) del(m)
		sleep(rand(90,110))
obj/Punch_Machine
...
obj/items/Nav_System
	...
	verb/Upgrade()
		...
		if(!Autopilot&&Upgrade_Level>=Max_Upgrade*0.35)
			Autopilot=1
...
obj/items/Force_Field
	...
	verb/Upgrade()
		...
		if(usr in view(1,src))
			...
			if(usr.last_attacked_time+900>world.time)
...
obj/items/Transporter_Watch
	...
	proc/Transport()
		...
		if(m&&m.z)
			...
			if(usr.grabbed_mob&&ismob(usr.grabbed_mob)&&usr.grabbed_mob.drone_module&&!usr.grabbed_mob.client) usr.Release_grab()
...
obj/items/Diarea_Injection
	...
	verb/Use()
		...
		player_view(15,usr)<<"[usr] injects [A] with a mysterious needle!"
		A.Diarea+=Level
		A.shit_virus_expire_time=world.realtime+(30*60*10)
...
obj/items/Fruit
	...
	verb/Eat()
		...
		if(usr.base_bp/usr.bp_mod<m.base_bp/m.bp_mod)
			usr.Attack_Gain((10*60)/((usr.Fruits_Eaten+1)**1.5),apply_hbtc_gains=0,include_weights=0)
			usr.Fruits_Eaten++
...
obj/items/Moon
	...
	verb/Use()
		...
		for(var/mob/A in player_view(12,src)) spawn if(A)
			if(Emitter&&A.ssjdrain>=300&&A.ssj2drain>=300&&A.SSj2Able&&!usr.SSj4Able&&usr.Race=="Saiyan") A.oozaru(50)
			else A.oozaru()
			if(Emitter&&!A.Tail) A.Tail_Add()
		spawn(100) if(src) del(src)
	verb/Upgrade()
		set src in view(1)
		var/obj/Resources/A=usr.resource_obj
		if(!A) return
...
mob/proc/max_weight()
	...
	var/n=(base_bp**0.3) * (adjusted_str**0.2) * 5
	return n
...
var/max_weights_mult=2.5
mob/proc/weights()
	if(!weights_obj||!weights_obj.suffix||weights_obj.loc!=src) return 1
	var/mult=1+(1.5*(weights_obj.weight/max_weight()))
	...
	Weights
		...
		verb/Choose_Weights_Icon()
			...
			if(old_suffix)
				...
				usr.Clothes_Equip(src)
		verb/Upgrade()
			var/cost_per_kilo=round(40/(usr.Intelligence**0.5))
			...
			if(usr in view(1,src))
				while(usr)
					if(suffix)
						...
						return
					switch(input(usr) in list("Cancel","Choose Icon","Change sharpness Level","Set damage style","Set silver blade"))
						if("Cancel") return
						if("Set silver blade")
...
obj/items/Senzu
	desc="This senzu will speed up your healing exponentially and heal you of all injuries and sicknesses. \
	...
	verb/Use()
		...
		if(!M.senzu_timer) M.Senzu_timer_countdown()
		M.senzu_timer=300
		M.Alter_regen_mult(3)
		M.Alter_recov_mult(3)
		M.Un_KO()
	...
	proc/Senzu_grow() spawn
		var/senzu_limit=200
		if(senzus.len>=senzu_limit)
...
mob/proc/Senzu_timer_countdown() spawn while(src)
	if(!senzu_timer) return
	senzu_timer--
	if(!senzu_timer) break
	else sleep(10)
mob/proc/Senzu_overload_countdown() spawn while(src)
...
obj/items/Focusin
	...
	desc="Injecting yourself with this will decrease your decline age by 5 years. But for 5 minutes you \
	will rapidly leech power from those around you. You must inject it into your eyes. \
	There is a 10% chance you will immediately die. You can only leech up to 90% of a person's BP \
	using this. Each use decreases zenkai mod by 10% and leech mod by 10%. The effects can be undone \
...
mob/proc/focusin_loop()
	spawn for(var/v in 1 to 300)
		for(var/mob/m in player_view(30,src)) if(base_bp+hbtc_bp<(m.base_bp+m.hbtc_bp)*0.9)
			var/old_bp=base_bp
