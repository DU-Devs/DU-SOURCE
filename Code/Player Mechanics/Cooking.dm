obj/proc/Fire_Cook()
	for(var/mob/Body/A in range(1,src))
		if(!A.Cooked)
			player_view(15,A)<<"[A.name] is cooked by the [src.name]"
			Cook(A)

proc/Cook_Check(mob/A) //Checks if a fire is nearby to make A cook
	var/obj/map_object/decor/light_emitter/L = locate() in view(1,A)
	if(L?.heated) return 1
