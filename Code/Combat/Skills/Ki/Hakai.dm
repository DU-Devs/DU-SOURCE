...
mob/proc
	CanUseHakai()
		...
		if(world.time - last_hakai_use < Limits.GetSettingValue("Hakai Cooldown"))
...
mob/proc
	BeginHakai(mob/m)
		...
		if(m)
			if(!hakai_failed)
				m.Death(src, Force_Death = 1, lose_immortality = 0)
	...
	StrongEnoughToHakai(mob/m)
		if(BP < m.BP * Limits.GetSettingValue("Hakai Advantage Requirement")) return
		return 1

proc/CheckHakaiDeleteCharacter(mob/m)
	set waitfor=0
	if(!m || !m.ckey || !Mechanics.GetSettingValue("Hakai Savewipe")) return
	var/key = m.ckey
	del(m)
