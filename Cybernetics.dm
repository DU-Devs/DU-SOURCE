...
obj/Body_Swap
	...
	verb/Body_Swap()
		...
		if(alignment_on&&both_good(usr,P))
		...
		if(P.Safezone)
			...
			return
		if(P.Health>30&&!P.KO)
			usr<<"[P] must have less than 30% health for you to steal their body"
			return
		if(ismob(usr.loc))
			usr<<"You can not jump into more than 1 person at a time"
			return
		if(!P||P=="Cancel") return
		player_view(15,usr)<<"[usr] tries to jump into [P]'s body!"
		var/mob/O=P.Duplicate()
		O.Savable=0
		for(var/mob/m in O) del(m)
		...
		usr.Get_Observe(O)
		P.Get_Observe(O)
		usr.Attack_Gain(500,P,apply_hbtc_gains=0,include_weights=0)
		...
		O.cyber_bp=usr.cyber_bp
		O.displaykey=usr.displaykey

		P.Alter_Res(-P.Res())
		for(var/obj/items/I in P.item_list) del(I)
		...
		Switch_Bodies(usr,O,save_override=1)
...
obj/Module
	...
	Giant_Version
		...
		0.75x offense and defense and speed"
		Giant=1;Strx=1.3;Endx=1.3;Offx=0.75;Defx=0.75;Spdx=0.75
	Body_Swap
		Cost=10000000
		Android_Only=1
		makes_toxic_waste=1
...
mob/proc/cyber_bp_Loop() while(src)
...
obj/Cybernetics_Computer
	...
	verb/Use()
		...
		if(usr in view(1,src))
			if(dna_protected&&dna_protected!=usr.Mob_ID)
			...
			switch(input("[src]") in options)
				...
				if("Repair Android/Cyborg")
					...
					/*if(usr.last_attacked_time+300>world.time)
...
mob/proc/Choose_Mob(T="Choose someone",list/L)
	...
	if(P) return P
mob/var/brain_transplant_allowed=1
				...
				if("Mind Swap")
					...
					for(var/mob/P in view(1,src))
						if(P!=usr&&P.brain_transplant_allowed&&!P.empty_player)
							if(!P.drone_module||(P.drone_module&&P.Is_drone_master(usr))||P.KO) Mobs+=P
					if(!(locate(/mob) in Mobs))
...
obj/items/Android_Blueprint
	...
	verb/Use() if(src in usr)
		if(Body&&usr.last_attacked_time+600>world.time)
		...
		for pure Androids, not Cyborgs.") in Options)
			...
			if("Mind Transfer")
				if(!usr.Android)
					...
					return
				var/list/Mobs=list("Cancel")
				for(var/mob/P in Get_step(usr,usr.dir)) if(P.Android&&!P.empty_player) Mobs+=P
				if(!(locate(/mob) in Mobs))
			...
			if("Repair Android/Cyborg")
				...
				/*if(P.last_attacked_time+300>world.time)
			...
			if("Upgrade")
				...
				if(P.ssj)
					...
					return
				if(P.client&&P!=usr&&(!P.KO||(alignment_on&&both_good(usr,P)))) switch(input(P,"[usr] wants to upgrade your cybernetic bp. Allow? \
...
mob/proc/Absorb_Blast(obj/Blast/B)
	...
	if(!Blast_Absorb||Ki>=max_ki*2||Disabled()) return
	if(world.realtime<blast_absorb_next_use) return
	var/Amount=0.5*(max_ki/100)*((B.BP*B.Force*B.percent_damage)/(BP*Res))
	if(B.Owner==src) Amount=0 //cant get energy from your own blasts
	Ki+=Amount
	if(Ki>max_ki*2)
		Explosion_Graphics(src,2)
		KO("blast absorb overload!",allow_anger=0)
...
mob/proc/Scrap_Absorb_Revert_Timer(obj/Scrap_Absorb/A)
	...
	if(!A) return
	spawn(900) if(src&&A&&A.Using) Scrap_Absorb_Revert(A)
