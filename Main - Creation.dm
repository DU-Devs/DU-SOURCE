mob/proc/Get_spawns()
	...
	if(Earth_Only)
		...
		if(Spawn_Bind) for(var/obj/Spawn/s in Spawn_List) if(s.z&&s.desc==Spawn_Bind&&!s.is_on_destroyed_planet())
			L+=s
		if(!L.len)
			for(var/obj/Spawn/s in Spawn_List) if(s.z&&!s.is_on_destroyed_planet())
				if(s.name==Race) L+=s
	return L
mob/proc/Go_to_spawn(First_time=0)
	if(world.maxz<5)
		loc=locate(1,1,1)
		return
	var/list/spawns=Get_spawns()
	if(!spawns.len)
		src<<"There are no spawns for your race, you have been sent to space."
		loc=locate(12,496,17)
		return
	...
	for(var/obj/Spawn_Redirector/SR in s.loc)
		if(SR.respawn_x)
			loc=locate(SR.respawn_x,SR.respawn_y,SR.respawn_z)
...
mob/proc
	Gender()
		if(!(Race in list("Majin","Bio-Android","Namek","Android")))
...
mob/proc
	...
	Skin()
		...
		if(Race=="Alien") Grid(Alien_Icons)
			...
			icon='Majin.dmi'
			Colorable=1
		else if(Race in list("Namek","Ancient Namek"))
			icon='Namek.dmi'
...
mob/proc/Choose_Hair(force_hair)
	...
	if(force_hair)
		...
		return
	if(dbz_character)
		src<<"Dragon Ball Z characters can not change their hair"
		return

	if((Race in list("Majin","Bio-Android","Namek","Android","Icer"))&&!icon) return
	Grid(Hairs)
var/list/Hairs=new
