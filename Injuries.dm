mob/verb/Injure()
	...
	if(P in epic_list)
		...
		return
	if(P in All_Entrants)
		src<<"You can not injure during tournaments"
		return
	Injury_Options(P)
mob/proc
	Injury_Options(mob/P)
		if(!P.KO&&P!=src)
			...
			return
		var/obj/Injuries/I
		switch(input(src,"What sort of injury do you want to inflict upon [P]?") in list("None","Arm","Leg",\
		"Eye","Internal","Brain","Dick"))
	...
	Inflict_Injury(mob/P,obj/Injuries/I)
		...
		for(var/obj/Injuries/O in P) if(O.type==I.type) Injuries++
		if(Injuries>=I.Max_Injuries)
			...
			their existing injuries of this type permanent if they aren't already?") in list("Yes","No"))
				if("Yes") if(P) for(var/obj/Injuries/O in P) if(O.type==I.type) O.Wear_Off=0
		else
			...
			P.contents+=I
			view(src)<<"[src] inflicts a [D] [I] injury on [P]"
			P.Add_Injury_Overlays()
mob/proc/Remove_Injury_Overlays() for(var/obj/Injuries/I in src) overlays-=I.icon
mob/proc/Add_Injury_Overlays()
	Remove_Injury_Overlays()
	for(var/obj/Injuries/I in src) overlays+=I.icon
mob/proc/Blood_Color()
	...
	if(Race=="Lunatak") return rgb(0,0,150)
	if(Race=="Majin") return rgb(150,0,75)
	return rgb(150,0,0)
obj/Injuries
	Del()
		var/mob/M=loc
		if(ismob(M)) M.overlays-=icon
		..()
	Givable=0
	Makeable=0
