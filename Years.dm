	...
	spawn for(var/mob/M in players) if(M.client&&M.loc)
		...
		if(round(Year,0.1)==round(Year))
			...
			if(M.oozaru_obj)
				...
				if(M.oozaru_obj.Setting) M.oozaru()
...
mob/proc/Age_Update()
	...
	if(Age>=13&&!(locate(/obj/Mate) in src))
		...
		if(Race in list("Majin","Bio-Android","Namek","Icer","Alien","Demon")) M.Asexual=1
		src<<"You gained the ability to mate."
	Gray_Hair()
mob/proc/decline_gains()
	var/n=decline_body_divisor()**0.6
	if(n<1) n=1
	if(n>2) n=2
	if(Age<Decline) n=1
	n=sqrt(n)
	...
	if(M.Asexual)
		...
		for(var/mob/P in Get_step(src,dir)) if(P.client)
			...
			if(Race==P.Race)
				...
				M.Race=input(Mother,"What race will the child be?") in parent_races
				if(!Mother) return
