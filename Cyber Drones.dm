...
mob/proc/Drone_bug_thingy() spawn while(src)
	if(!z) return
	if(!client)
		if(Health>100) Health=100
		if(Ki>max_ki) Ki=max_ki
	...
	if(ismob(P))
		...
		while(Instance==Drone_Order && P && Target==P)
			...
			if(P.locz()!=z || P.get_area()!=get_area()) break
				...
				if(P.KB || P.Ship || world.time>=last_target_position_update_time+To_multiple_of_one(3.7))
					last_target_position_update_time=world.time
					last_known_target_position=P.True_Loc()
				...
				if(get_dist(src,last_known_target_position)>1)
					...
					if(!P.Flying) Land()
				if(P && P.True_Loc()==Get_step(src,dir))
					var/sub_target=P
					if(P.Ship) sub_target=P.Ship
					Melee(sub_target)
					if(P && P.KO && (!P.Dead||z!=5))
						if(getdist(src,P.True_Loc())>1) for(var/v in 1 to 30)
							drone_step(P,ignore_master=1,from_drone_attack=1)
							sleep(world.tick_lag)
							if(!P || getdist(src,P.True_Loc())<=1) break
						if(P && getdist(src,P.True_Loc())<=1) Drone_steal_from_player(P)
						if(lethal && P && getdist(src,P.True_Loc())<=1)
...
mob/proc/drone_step(mob/P,ignore_master,from_drone_attack,avoid_caves=1)
	...
	if(drone_pathfind_steps<=0)
		if(P)
			var/turf/t=Get_step(src,get_dir(src,P.True_Loc()))
			if(avoid_caves && istype(t,/turf/Teleporter))
				...
				if(!(locate(/obj/Turfs/Door) in t))
					step_towards(src,P.True_Loc())
					if(loc==Former_Loc)
						sleep(2)
						Fly()
						if(P) step_towards(src,P.True_Loc())
			if(loc==Former_Loc)
				//stored_path=get_path(src,P.True_Loc())
		...
		density=1
		if(P)
			var/turf/next_step=get_step_to(src,P.True_Loc())
				...
				if(next_step && (next_step.FlyOverAble || Drone_can_break_this_wall(next_step)))
					...
					if(attack_turf!=P)
						if(Drone_can_break_this_wall(attack_turf))
							...
							var/turf/p_original_loc=P.True_Loc()
	...
	if(Target && viewable(src,Target.True_Loc(),1))
		dir=get_dir(src,Target.True_Loc())
		drone_pathfind_steps=0
...
mob/proc/Collect_Resources(Max,mob/P)
	...
	while(src&&D&&D.loc==src&&D.suffix&&("Gather" in D.Commands)&&Instance==Drone_Order)
		...
		if(P&&P.locz()==z&&P.get_area()==get_area()&&Res()>=Max)
			...
			while(P&&P.locz()==z&&Instance==Drone_Order&&P.get_area()==get_area()&&(getdist(src,P.True_Loc())>4||!(P in player_range(4,src)))&&("Gather" in D.Commands))
				drone_step(P,ignore_master=1)
				sleep(1)
			if(P&&getdist(src,P.True_Loc())<=4&&(P in player_range(4,src)))
		...
		if(P&&P.locz()==z&&P.get_area()==get_area()&&Res()>=Max)
			...
			if(Stealables)
				...
				if(Instance==Drone_Order)
					...
					else if(ismob(M)&&M.Res()>=drone_res_steal_amount*Resource_Multiplier&&getdist(src,M.True_Loc())<=5&&viewable(src,M)&&("Steal" in D.Commands)&&!Master_is_near()&&!Is_drone_master(M))
						var/Timer=7
...
mob/proc/Drone_steal_from_player(mob/m)
	if(m.KO && m.locz()==z)
		var/turf/t=Get_step(m.True_Loc(),src)
		if(!t||!isturf(t)) return
		loc=t
		dir=get_dir(src,m.True_Loc())
		sleep(10)
...
mob/proc/Binded_at_bind_spawn()
	if(!bind_spawn||z!=6||getdist(src,bind_spawn)>20) return
	for(var/obj/Curse/c in bind_objects) if(c.loc==src) return 1
...
mob/proc/Drone_Genocide(R,mob/P) //R=Race to kill
	...
	while(src&&D&&D.loc==src&&D.suffix&&("Genocide" in D.Commands)&&Instance==Drone_Order)
		...
		if(Mobs.len)
			...
			var/Steps=500
			Fly()
			while(M&&M.locz()==z&&Steps&&(getdist(src,M.True_Loc())>5||!viewable(src,M))&&Instance==Drone_Order&&M.get_area()==get_area())
				Steps--
				drone_step(M,ignore_master=1)
				sleep(1)
			Land()
			if(Instance==Drone_Order) if(ismob(M)&&M.locz()==z&&getdist(src,M.True_Loc())<=8&&viewable(src,M)&&!Is_drone_master(M))
				if(M.client)
...
mob/proc/Drone_Patrol(mob/PP)
	...
	while(src&&D&&D.loc==src&&D.suffix&&("Patrol" in D.Commands)&&Instance==Drone_Order)
		...
		if(Mobs.len)
			...
			var/Steps=500
			var/range=5
			while(Instance==Drone_Order&&P&&P.locz()==z&&Steps&&P.get_area()==get_area()&&(getdist(src,P.True_Loc())>range||!viewable(src,P)))
				Steps--
				drone_step(P,ignore_master=1)
...
mob/proc/Get_max_cyber_bp_upgrade(race)
	var/n=Knowledge*(usr.Intelligence**0.1)*cyber_bp_mod
