var
	...
	list
		...
		destroyable_planets=list("Android","Arconia","Atlantis","Vegeta","Desert","Earth","Ice",\
		"Jungle","Namek") //also contains when the planet was last destroyed
...
mob/Admin4/verb
	set_planet_destroy_immunity_time()
		set category="Admin"
		planet_destroy_immunity_time=input(src,"Set the amount of hours a planet will be immune to planet \
		destroy after it has last been destroyed","options",planet_destroy_immunity_time) as num
	set_planet_destroy_bp_requirement()
		set category="Admin"
		planet_destroy_bp_requirement=input(src,"Set the % of BP compared to the strongest person's BP \
		online that a player needs to destroy a planet","options",planet_destroy_bp_requirement) as num
		if(planet_destroy_bp_requirement<1) planet_destroy_bp_requirement=1
...
proc/destroy_planet(planet,power=1)
	...
	spawn for(var/v in 1 to 300)
		for(var/mob/m in players) if(m.get_area()==a&&prob(30))
			spawn if(m) m.Screen_Shake(10,8)
		sleep(10)
		...
		for(var/mob/m in players) if(m.get_area()==a&&prob(10))
			...
			if(L.len)
				...
				for(var/mob/m2 in view(2,t))
					...
					m2.Health-=50*(power/m2.BP)
					if(m2.Health<=0) spawn if(m2) m2.Death("random explosion!")
				spawn for(var/turf/TT in view(3,t)) if(TT.Health<power)
					TT.Health=0
					TT.Destroy()
					sleep(To_tick_lag_multiple(0.5))
		sleep(10)
...
proc/restore_planet(planet)
	...
	if(planet=="Vegeta"&&("Atlantis" in destroyed_planets)) restore_planet("Atlantis")
	if(planet=="Atlantis"&&("Vegeta" in destroyed_planets)) restore_planet("Vegeta")
...
proc/Ship_on_destroyed_planet_loop() spawn while(1)
