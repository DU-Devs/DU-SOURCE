var/obj/Planet_Vegeta_Fitness_Objects/Lord_Freeza/lord_freeza_obj

obj
	Planet_Vegeta_Fitness_Objects
		...
		Lord_Freezas_Monkey_Death_Trap_Emporium
			icon='Lord Freezas Monkey Death Trap Emporium.jpg'
			desc="Totally not a death trap to kill filthy monkey scum. COME ON IN!"
		Planet_Vegeta_Fitness_Logo
			icon='planet vegeta fitness logo.jpg'
		...
		Lord_Freeza
			...
			"Welcome filthy monkeys to Lord Freeza's Monkey Death Tra- I mean Planet Vegeta Fitness",\
			...
			proc/Death_ball_kill(mob/m,idle_timer=40)
				...
				var/obj/death_ball=new/obj/Planet_Vegeta_Fitness_Objects/Lord_Freeza_Death_Ball_Graphic(loc)
				sleep(idle_timer)
				player_view(44,usr)<<sound('basicbeam_fire.ogg',volume=15)
				for(var/v in 1 to 70)
					if(!m || m.z!=z) break
					else
			...
			if(viewable(t,origin,20))
				spawn for(var/v in 1 to 4)
					for(var/mob/m in t) if(m.z)
						...
						m.Health-=dmg
						if(m.Health<=0)
							spawn if(m) m.Death("nuclear explosion",really_dead,lose_hero=0,lose_immortality=0)
						sleep(rand(3,5))
