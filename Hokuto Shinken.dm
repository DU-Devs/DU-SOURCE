		...
		while(Amount)
			...
			if(Chosen_Targets)
				...
				if(M&&M.z==usr.z)
					...
					if(M&&!M.KO) usr.Melee()
					spawn if(usr && prob(40)) Make_Shockwave(usr,sw_icon_size=pick(64,128))
			sleep(3)
		for(var/mob/m in players) for(var/obj/Hokuto_Shinken_Energy/E in m)
			if(E.Creator==usr.key) spawn if(m) m.Hokuto_Shinken_Effects(usr)
...
mob/proc/Hokuto_Shinken_Effects(mob/P)
	for(var/obj/Hokuto_Shinken_Energy/E in src) if(E.Creator==P.key)
		var/hs_power=((E.Level/BP)**0.4 + (E.hs_str/End)**0.3) / 2 * 0.75
		if(hs_power<1)
