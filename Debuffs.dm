...
mob/proc
	...
	Good_attack_good_loop() spawn
		while(src)
			if(Opponent&&Opponent!=src) Opponent.Good_attacking_good_against_their_will(src)
			if(!client) sleep(600)
			else sleep(30)
	...
	Check_if_being_teamed(mob/a) //src = person being attacked. a = attacker
		...
		for(var/mob/m in player_view(20,src))
			if(m.client&&m!=src&&!m.KO&&m.z==z&&m.Extrapolated_target_is(src,hit_req=8,min_time=55,max_time=300))
				attackers+=m
	...
	Set_possible_teamer_list(mob/M) //M is the attacker. called in set_opponent()
	...
	Is_teamer()
		var/area/a=get_area()
		if(!a) return
		for(var/mob/m in a.player_list) if(m!=src&&m.client&&getdist(src,m)<60)
...
mob/proc
				...
				if(chaser_still_chasing)
					...
					if((getdist(src,chaser)>=25 && !running_into_caves && z==chaser.z) || running_into_caves)
						if(chaser.chaser==src && chaser.fearful)
							...
							if(chaser.Health>Health*0.8 || (chaser.key in anger_reasons)) //cant really fear someone who isn't beating you
								...
								if(!Opponent || Opponent==chaser || !Opponent.client || getdist(src,Opponent)>25)
									return 1
	Get_chaser_from_key()
	...
	Chase_over()
		...
		if(!is_runner && !is_teamer && chaser.Opponent && chaser.Opponent!=src && chaser.Opponent.client && chaser.Opponent.Opponent==chaser)
