...
mob/Troll
	...
	proc/Troll_Grab_Struggle() while(src)
		var/mob/A
		if(Is_Grabbed()) for(var/mob/P in view(1,src)) if(P.grabbed_mob==src) A=P
		if(A)
	...
	proc/Troll_Spam()
		...
		while(src)
			...
			sleep(rand(0,10))
	proc/Troll_Name()
		var/list/L=list("Carrot_Man","gyaku","prince","killer","ss5","ahhh","naruto","adeetwet","0","piccolo","Vegeta",\
