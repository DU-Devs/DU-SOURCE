...
mob/Admin5/verb/multiply_transform_size(mob/m in world)
	set category="Admin"
...
proc/player_view(range=20,mob/center)
	var/list/l=new
	var/area/a=locate(/area) in range(0,center)
	if(!a) return l
	for(var/mob/m in a.player_list) if(m&&m.z==center.z&&getdist(m,center)<=range&&viewable(m,center)) l+=m
	return l

proc/mob_view(range=20,mob/center)
	var/list/l=new
	var/area/a=locate(/area) in range(0,center)
	if(!a) return l
	for(var/mob/m in a.mob_list) if(m.z==center.z&&getdist(m,center)<=range&&viewable(m,center)) l+=m
	return l

proc/npc_view(range=20,mob/center)
	var/list/l=new
	var/area/a=locate(/area) in range(0,center)
	if(!a) return l
	for(var/mob/m in a.npc_list) if(m.z==center.z&&getdist(m,center)<=range&&viewable(m,center)) l+=m
	return l

proc/viewable(mob/a,mob/b,max_dist=5000)
	...
	while(t&&t!=b)
		...
		if(!t||t.opacity) break
		else for(var/obj/o in t) if(o.opacity) break
	if(!t||t!=b) return
	return 1
...
mob/Admin4/verb/Toggle_toxic_waste()
	set category="Admin"
	toxic_waste_on=!toxic_waste_on
	if(toxic_waste_on) src<<"Toxic waste is now on"
	else src<<"Toxic waste is now off"
	...
	New()
		spawn if(src)
			...
			if(!toxic_cloud_icon)
				...
				var/obj/o=new
				o.icon=i
				o.Enlarge_Icon(Get_Width(o.icon)*2,Get_Height(o.icon)*2)
...
mob/proc/Geiger_sound_loop() spawn if(src)
	...
	while(in_radiation)
		...
		sleep(173)
	geiger_sounding=0
mob/proc/Radiation_loop() spawn if(src)
	while(src)
		in_radiation=0
		if(Dead)
			...
			if(!Tournament||z!=7||!(src in All_Entrants))
				for(var/obj/Toxic_Cloud/tc in toxic_clouds)
					if(z==tc.z&&getdist(src,tc)<=tc.toxicity_range&&viewable(src,tc))
						in_radiation=1
						if(!Dead&&!(Race in list("Majin","Android","Bio-Android","Demon"))) radiation_level+=8
						break
...
mob/proc
	Apply_stun(n=1,t=5)
		if(knockback_immune) return
		if(world.time<stun_immunity) return
		n=To_multiple_of_one(n/stun_resistance_mod)
		t=To_multiple_of_one(t)
		if(stun_level<n) stun_level=n
		if(stun_time<t) stun_time=t
		Stun_wearoff()
	Stun_wearoff() spawn if(src)
		if(stun_loop) return
		stun_loop=1
...
mob/proc/Kilt_by_redneck()
	...
	for(var/v in 1 to 100)
		if(!m) return
		m.loc=Get_step(m,get_dir(m,src))
	...
	sleep(35)
	for(var/mob/a in player_view(20,m)) a.Screen_Shake(300)
	sleep(35)
...
proc/Remove_all_nulls() spawn while(1)
...
obj/Car
	...
	Move()
		//var/turf/t=loc
		//if(t&&isturf(t)) t.Temporary_turf_overlay('Damaged Ground.dmi',5*600)
...
proc/Car_wreck_loop() spawn(3000)
	while(1)
