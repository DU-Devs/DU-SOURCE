...
mob/proc/Buff_Drain_Loop() spawn
...
mob/proc
	...
	Buff_Enable(obj/Buff/O) if(!O.being_edited&&!Redoing_Stats)
		...
		/*for(var/obj/Kaioken/k in src) if(k.Using)
			src<<"Buffs can not be combined with Kaioken"
			return*/
		...
		rebuff_timer=20
	Rebuff_timer_countdown() spawn while(src)
		...
		spawn for(var/O in 1 to N)
			switch(V)
				if("Shockwave")
					...
					sleep(rand(3,15))
				if("Shaking")
					for(var/mob/M in player_view(20,src)) if(M.client) M.Screen_Shake()
					sleep(rand(3,15))
				if("Damaged Ground")
					for(var/turf/T in Turf_Circle(EA["Range"],src))
						T.Make_Damaged_Ground(1)
						if(prob(25)) sleep(1)
					sleep(rand(5,15))
				if("Dust")
					for(var/turf/T in view(2,src))
						Dust(T,To_multiple_of_one(3))
						if(prob(25)) sleep(1)
					sleep(rand(5,15))
				if("Explosions")
					var/list/Temp=new
