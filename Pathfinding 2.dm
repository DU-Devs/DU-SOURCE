...
*/
/*mob/Admin3/verb/Pathfind(mob/P in world)
	while(src)
		G_step_to(P)
			...
			if(!tdest)
				var
					list
						...
						l3 = list()
						l4 = list()
					mob/G_Step_Test/S = new
				S.density = owner.density
				for(var/turf/warp/W in range(10,owner)) if(W.dest) if(W.dest.dest == W)
					if(owner.loc == W || get_step_to(owner,W))
						l += W
					else if(owner.multitile) for(var/atom/movable/M in owner.components)
						if(M.loc == W || get_step_to(M,W))
							l += W
							break
				for(var/turf/warp/W in range(10,dest)) if(W.dest) if(W.dest.dest == W)
					S.loc = dest
					if(S.loc == W || get_step_to(S,W))
						if(W.dest in l)
							tdest = W.dest
							goto Search
						else
							...
							l4 += W
				while(l2.len)
					var/turf/warp/W = l2[1]
					for(var/turf/warp/W2 in range(10,W)-l2-l3) if(W2.dest) if(W2.dest.dest == W2)
						S.loc = W
						if(S.loc == W2 || get_step_to(S,W2))
							if(W2.dest in l)
								tdest = W2.dest
								goto Search
							else
					...
					l2.Cut(1,2)
					l3 += W
				for(var/turf/warp/W in l4)
					if(W.dest.z == owner.z)
						tdest = W.dest
						goto Search
					for(var/turf/warp/W2 in l) if(W.dest.z == W2.dest.z)
						tdest = W2
						goto Search
				if(owner.z == dest.z)
					tdest = dest
					goto Search
				for(var/turf/warp/W in l) if(W.dest.z == dest.z)
					tdest = W
					goto Search
				Search
				S.loc = null
...
proc
	...
	G_get(atom/movable/A,atom/movable/B,function,x_off,y_off) if(isloc(A,B)&&!isarea(A)&&!isarea(B))
		...
		if(!isturf(A))
			...
			if(A.multitile)
				la = list()
				for(var/atom/C in A.components) la += C.loc
			else A = A.loc
		if(!isturf(B))
			while(!isturf(B.loc))
				if(!B.loc) return
			...
			if(B.multitile)
				lb = list()
				for(var/atom/C in B.components) lb += C.loc
			else B = B.loc
		switch(function)
			if(1)
	...
	proc
		...
		Possible_Path(turf/T,turf/W)
			if(multitile) for(var/atom/movable/C in components)
				var/turf/T2 = locate(T.x+C.x-x,T.y+C.y-y,T.z)
				if(!T2) return
				if(T2.dest) if(!can_warp||(W&&T2!=W)) return
				else if(density&&T2.density) return
			else
		...
		G_warp(turf/T,turf/Old)
			...
			if(multitile)
				for(var/turf/T2 in range(3,T)) if(G_getdist(src,T,T2.x-x,T2.y-y) == 1) if(Can_Enter(T2))
					for(var/atom/movable/C in components)
						C.loc = locate(T2.x+C.x-x,T2.y+C.y-y,T2.z)
					loc = T2
					dir = G_get_dir(T,src)
					G_adjust_icon()
					return
