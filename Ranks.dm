mob/proc/Give_Rank(mob/A)
	set category="Admin"
	var/list/Planets=list("Cancel","Earth","Namek","Vegeta","Arconia","Ice Planet","Heaven","Hell",\
	"Android Skill Master")//,"Z Character Skill Sets")
	var/list/Ranks=new
	switch(input(src,"Choose Planet Rank") in Planets)
		if("Cancel") return
		if("Z Character Skill Sets")
			switch(input(src,"What skill set?") in list("Cancel","Carrot_Man","Piccolo","Blowhan","Vegeta","Trunks","Tien",\
		...
		if("Namek")
			...
			switch(input(src,"What Rank?") in Ranks)
				...
				if("Teacher") A.Namek_Teacher(src)
		if("Vegeta")
...
mob/proc/Rank_Check()
	...
	var/list/rank_tags=list("Elder","Daimao","Cardinal Kai","North Kai","Kaioshin","Yardrat","Namek Teacher",\
			...
			if("Yes")
				if(Rank_taken(V)) src<<"Someone accepted the rank before you"
					...
					switch(V)
						...
						if("Namek Teacher") Namek_Teacher()
						if("Elder") Elder()
...
mob/proc/Race_can_have_rank(rank)
	switch(rank)
		...
		if("Namek Teacher") if(Race!="Namek") return
		if("Elder") if(Race!="Namek") return
...
mob/proc/give_hbtc_key()
	var/obj/items/Door_Pass/D=new(src)
	D.name="Time Chamber Key"
	D.icon='Key.dmi'
	D.Password=7125
	D.Cost=2400000*Resource_Multiplier
	D.can_blueprint=0
	D.clonable=0
	D.Stealable=1
	D.Cost=3000000
...
mob/proc
	Guardian(mob/P)
		...
		if(Race=="Namek") contents.Add(new/obj/Make_Dragon_Balls)
	...
	Elder(mob/P)
		if(P) Log(P,"[P.key] gave [key] Namek Elder")
		Can_Remake=0
		Ranks+="Elder"
		...
		new/obj/Meditate_Level_2,new/obj/Shadow_Spar,new/obj/Namek_Fusion,new/obj/Hide_Energy)
		if(Race=="Namek") contents.Add(new/obj/Make_Dragon_Balls,new/obj/Reincarnation)
		contents.Add(new/obj/Telepathy,new/obj/Observe,new/obj/Sense,new/obj/Advanced_Sense)
		contents+=new/obj/RankChat
		src<<"<font color=yellow>You were given the Namek Elder rank"
	Namek_Teacher(mob/P)
		if(P) Log(P,"[P.key] gave [key] Namek Teacher")
		Ranks+="Namek Teacher"
		...
		contents.Add(new/obj/Telepathy,new/obj/Observe,new/obj/Shadow_Spar,new/obj/Namek_Fusion)
		Attack_Gain(1000,apply_hbtc_gains=0,include_weights=0)
		src<<"<font color=yellow>You were given the Namek Master rank"
	Elite_Alien(mob/P)
...
mob/proc/Vegeta(mob/P)
	if(P) Log(P,"[P.key] gave [key] Vegeta Skills")
	contents.Add(new/obj/Attacks/Explosion,new/obj/Attacks/Final_Flash,new/obj/Attacks/Galic_Gun,\
...
mob/proc/Cell(mob/P)
	...
	contents.Add(new/obj/Attacks/Kamehameha,new/obj/Attacks/Piercer,new/obj/Attacks/Death_Ball,\
